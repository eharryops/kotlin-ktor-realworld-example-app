...
class UserRepository {
    ...
    fun getUserStats(userId: Long): UserStats {
        return transaction {
            val articlesCount = Articles.select { Articles.author eq userId }.count()
            val commentsCount = Comments.select { Comments.author eq userId }.count()
            val favoritesCount = ArticleFavorites.select { ArticleFavorites.user eq userId }.count()
            UserStats(articlesCount, commentsCount, favoritesCount)
        }
    }
}
